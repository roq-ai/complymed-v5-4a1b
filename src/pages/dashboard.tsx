...
function DashboardPage() {
  ...
  const organizationStat = useSWR<PaginatedInterface<OrganizationInterface>>(
    () => `/organizations`,
    () => getOrganizations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Organizations',
      result: organizationStat?.data?.totalCount,
      isLoading: organizationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
